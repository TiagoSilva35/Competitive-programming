...
bool valid(int i, int j){
    return (i >= 0 && j >= 0) && (i < (int)matrix.size() && j < (int)matrix[0].size()) && (matrix[i][j] == '.' || matrix[i][j] == 'X');
}
...
void combs(vi &v, int idx, int n, vector<bool> visited, map<int,string> &m){
    if ((int)temp_comb.size() == n){
        ...
        forn(g,h){
            forn(y,w){
                if (matrix[g][y] == 'O') kk1 = y;
                if (matrix[g][y] == 'X') kk2 = y;
            }
        }
        ff(0,kk1,visited1);
        /*for(auto temp1:visited1){
            for(auto temp2:temp1){
                cout << temp2 << " ";
            }
            cout << endl;
        }*/
        if(visited1[h-1][kk2] == true){
            for(auto k:matrix){
                cout << k << endl;
            }
        }
...
void solve(){
    ...
    forn(i,h){
        string temp;
        cin >> temp;
        if(find(all(temp),'O') != temp.end()){
            matrix[0] = temp;
        }
        else if(find(all(temp),'X') != temp.end()){
            matrix[h-1] = temp;
        }
        else{
            m[k++] = temp;
