...
bool intersect(Line2D l1, Line2D l2, Point2D& p) {                                       // Check if lines intersect. If so, p is the intersection point
  T d = cross(l1.v, l2.v);
...
bool in_polygon(const std::vector<Point2D>& polygon, Point2D p) {
  ...
  T sum = 0;
  for (int i = 0; i < n; i++) {
    if (on_line(Line2D(polygon[i], polygon[(i + 1) % n]), p)) return true;
